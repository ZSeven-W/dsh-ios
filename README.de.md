...
<p align="center">
  ...
  <sub>21 Agenten-Tools &bull; Live-MJPEG-Seitenleistenpanel &bull; Simulator &amp; echtes iPhone über USB &bull; Listen-/Feed-Zeilenaktionen &bull; SwiftUI-Preview-Hot-Reload</sub>
</p>

<p align="center">
  <sub>npm: <code>@zseven-w/dsh-ios</code> &middot; Aktuelles Plugin-Release: <code>0.1.0-rc.1</code> &middot; Getestet mit DSH <code>0.1.0-rc.6</code></sub>
</p>
...
<p align="center">
  <sub>rc: <code>0.1.0-rc.1</code> ist noch nicht auf npm veröffentlicht — siehe <a href="#in-dsh-installieren">Installieren</a></sub>
</p>
...
| 🛠️ **21 Agenten-Tools** | Geräte, Starten/Herunterfahren, Screenshot, Interaktion, Build &amp; Run, Unified Logs, AXe-basierter UI-Tree + Tippen per Element, Listen-/Feed-Zeilenaktionen, Vision-OCR-Suche/-Tippen, SwiftUI-Preview-Hot-Reload, Prozesse, Backtrace, Leaks, App-Infos. |
...
| 🔐 **Nur-Loopback-Transport** | serve-sim bindet 127.0.0.1 in einem dedizierten Portbereich; jede Route verlangt einen Loopback-Peer, einen Loopback-`Host` sowie Fetch-Metadata-/Origin-Prüfungen; HMAC-Capabilities laufen innerhalb von 10 Minuten ab. Die WebDriverAgent-Control-/MJPEG-Tunnel auf einem echten Gerät sind Loopback-`iproxy`-Weiterleitungen unter derselben Absicherung. |
...
- **Echtgeräte-Transport** — die WebDriverAgent-Tunnel für Control (REST, Geräteport 8100) und Screen (MJPEG, Port 9100) sind Loopback-`iproxy`-Weiterleitungen über die USB-Verbindung; sie liegen hinter derselben Signierte-Routen-Absicherung, und der Browser spricht weiterhin ausschließlich mit dem DSH-Webserver-Ursprung.
...
- **DSH ≥ 0.1.0-rc.6 mit Web-Bundle** für das Panel. Headless-Profile funktionieren ebenfalls: Alle 21 Tools arbeiten normal, nur ohne die Live-Ansicht.
- **Nicht-macOS-Hosts**: Das Plugin lädt und alle 21 Tools registrieren sich, aber jeder Aufruf gibt einen erklärenden Fehler zurück (`iOS Simulator requires macOS with Xcode …`).
...
- **lldb-Attach** benötigt den macOS-Entwicklermodus: Führen Sie einmal `sudo DevToolsSecurity -enable` aus. Bis dahin verwendet `ios_sim_backtrace` Xcodes `sample`-Engine (nicht suspendierend) und `ios_sim_leaks` läuft mit dem Aktivierungshinweis in degradierter Form.
- **Echtes iPhone** — ein per USB verbundenes iPhone mit entsperrtem Bildschirm (WebDriverAgent kann bei gesperrtem Bildschirm nicht starten; erwägen Sie „Automatische Sperre: Nie“), ein datenfähiges USB-Kabel (eine reine WLAN-Kopplung kann die Port-Weiterleitung nicht tragen), auf dem Gerät aktivierter Entwicklermodus, ein WebDriverAgent-Checkout unter `~/Library/Caches/dsh-ios/wda/src` (das Plugin baut dessen `WebDriverAgentRunner`-Scheme von dort — es lädt nie etwas herunter und klont nichts) sowie `iproxy` aus libimobiledevice (`brew install libimobiledevice`) für die USB-Tunnel. Der erste WDA-Build installiert einen signierten WebDriverAgentRunner: Vertrauen Sie bei der entsprechenden Aufforderung dem Zertifikat auf dem Gerät und führen Sie `ios_real_start_wda` erneut aus, wenn das Free-Team-Signaturprofil abläuft (Lebensdauer 7 Tage).
...
> **rc-Hinweis** — `0.1.0-rc.1` ist noch nicht auf npm veröffentlicht. Installieren Sie bis dahin den gepackten Tarball:
>
> ```sh
...
- **Echtgeräte-Aufrufe schlagen mit einem codierten Status fehl** — der Status des Panels benennt die Ursache, statt zu raten: `device-locked` (Telefon entsperren; es erholt sich von selbst), `cert-untrusted` (dem WebDriverAgent-Zertifikat auf dem Gerät vertrauen), `profile-expired` (die Free-Team-Signierung hält 7 Tage — `ios_real_start_wda` erneut ausführen, um neu zu bauen), `tunnel-failed` (USB-Verbindung/iproxyd prüfen), `device-unplugged` (ein datenfähiges USB-Kabel verwenden — eine reine WLAN-Kopplung kann die Port-Weiterleitung nicht tragen).
