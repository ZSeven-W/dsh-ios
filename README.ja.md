...
| 🔐 **ループバック限定の転送** | serve-sim は専用ポート範囲の 127.0.0.1 にのみバインドします。全ルートでループバックピア、ループバック `Host`、Fetch-Metadata/Origin チェックが必須で、HMAC ケーパビリティは 10 分で失効します。実機上の WebDriverAgent 制御/MJPEG トンネルも、同じガードの内側にあるループバック `iproxy` フォワードです。 |
...
- serve-sim はループバックのみで専用ポート範囲（3181–3244）のフォアグラウンド子プロセスとして動くため、ユーザー自身のポート 3100 の serve-sim には決して触れません。`--host` は使いません。
- **実機転送**——WebDriverAgent の制御（REST、デバイスポート 8100）と画面（MJPEG、ポート 9100）のトンネルは、USB リンク上のループバック `iproxy` フォワードです。同じ署名付きルートのガードの内側にあり、ブラウザが通信するのは依然として DSH ウェブサーバーのオリジンだけです。
...
- **実機 iPhone**——画面がロック解除された USB 接続の iPhone（ロック画面では WebDriverAgent を起動できません。自動ロックは「しない」を検討）、データ転送対応の USB ケーブル（Wi-Fi のみのペアリングではポートフォワードを運べません）、デバイスで有効化された開発者モード、`~/Library/Caches/dsh-ios/wda/src` にある WebDriverAgent チェックアウト（プラグインはそこから `WebDriverAgentRunner` スキームをビルドします——何かをダウンロードしたりクローンしたりすることはありません）、そして USB トンネル用の libimobiledevice の `iproxy`（`brew install libimobiledevice`）が必要です。最初の WDA ビルドでは署名済みの WebDriverAgentRunner がインストールされます。プロンプトに従ってデバイスで証明書を信頼し、無料チームの署名プロファイルが失効したら（7 日間の有効期間）`ios_real_start_wda` を再実行してください。
...
- **実機の呼び出しがコード化されたステータスで失敗する**——パネルのステータスは推測ではなく原因を示します。`device-locked`（電話機のロックを解除。自動的に回復します）、`cert-untrusted`（デバイスで WebDriverAgent の証明書を信頼）、`profile-expired`（無料チームの署名は 7 日間——`ios_real_start_wda` を再実行して再ビルド）、`tunnel-failed`（USB リンク/iproxyd を確認）、`device-unplugged`（データ転送対応の USB ケーブルを使用——Wi-Fi のみのペアリングではポートフォワードを運べません）。
