...
| 🔐 **Transport en boucle locale uniquement** | serve-sim se lie à 127.0.0.1 sur une plage de ports dédiée ; chaque route exige un pair en boucle locale, un `Host` en boucle locale et des vérifications Fetch-Metadata/Origin ; les capacités HMAC expirent sous 10 minutes. Les tunnels de contrôle/MJPEG de WebDriverAgent sur un appareil réel sont des redirections `iproxy` en boucle locale derrière la même barrière. |
...
- **Transport des appareils réels** — les tunnels de contrôle (REST, port d'appareil 8100) et d'écran (MJPEG, port 9100) de WebDriverAgent sont des redirections `iproxy` en boucle locale sur la liaison USB ; ils se trouvent derrière la même barrière de routes signées, et le navigateur ne parle toujours qu'à l'origine du serveur web de DSH.
...
- **l'attachement lldb** nécessite le mode développeur de macOS : exécutez `sudo DevToolsSecurity -enable` une fois. D'ici là, `ios_sim_backtrace` utilise le moteur `sample` de Xcode (non suspensif) et `ios_sim_leaks` se dégrade avec l'indication pour l'activer.
- **iPhone réel** — un iPhone connecté par USB avec l'écran déverrouillé (WebDriverAgent ne peut pas démarrer sur un écran verrouillé ; pensez à Verrouillage auto : Jamais), un câble USB capable de transporter des données (un appairage Wi-Fi seul ne peut pas porter le transfert de port), le mode développeur activé sur l'appareil, une copie de WebDriverAgent dans `~/Library/Caches/dsh-ios/wda/src` (le plugin y compile son scheme `WebDriverAgentRunner` — il ne télécharge ni ne clone jamais rien) et `iproxy` de libimobiledevice (`brew install libimobiledevice`) pour les tunnels USB. La première compilation de WDA installe un WebDriverAgentRunner signé : faites confiance à son certificat sur l'appareil quand vous y êtes invité, et relancez `ios_real_start_wda` quand le profil de signature d'équipe gratuite expire (durée de vie de 7 jours).
...
- **Les appels sur appareil réel échouent avec un état codé** — l'état du panneau nomme la cause au lieu de deviner : `device-locked` (déverrouillez le téléphone ; il récupère tout seul), `cert-untrusted` (faites confiance au certificat WebDriverAgent sur l'appareil), `profile-expired` (la signature d'équipe gratuite dure 7 jours — relancez `ios_real_start_wda` pour recompiler), `tunnel-failed` (vérifiez la liaison USB/iproxyd), `device-unplugged` (utilisez un câble USB capable de transporter des données — un appairage Wi-Fi seul ne peut pas porter le transfert de port).
