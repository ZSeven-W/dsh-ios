...
| 🔐 **Transporte somente por loopback** | o serve-sim faz bind em 127.0.0.1 em uma faixa de portas dedicada; toda rota exige um par em loopback, um `Host` de loopback e verificações de Fetch-Metadata/Origin; as capabilities HMAC expiram em até 10 minutos. Os túneis de controle/MJPEG do WebDriverAgent em um dispositivo real são encaminhamentos `iproxy` de loopback sob a mesma proteção. |
...
- **Transporte de dispositivo real** — os túneis de controle do WebDriverAgent (REST, porta 8100 do dispositivo) e de tela (MJPEG, porta 9100) são encaminhamentos `iproxy` de loopback sobre o link USB; eles ficam atrás da mesma proteção de rotas assinadas, e o navegador continua falando apenas com a origem do webserver do DSH.
...
- **lldb attach** precisa do Modo de Desenvolvedor do macOS: execute `sudo DevToolsSecurity -enable` uma vez. Até lá, `ios_sim_backtrace` usa o motor `sample` do Xcode (não suspensivo) e `ios_sim_leaks` degrada com a dica de habilitação.
- **iPhone real** — um iPhone conectado via USB com a tela desbloqueada (o WebDriverAgent não inicia com a tela bloqueada; considere Bloqueio Automático: Nunca), um cabo USB com capacidade de dados (um pareamento somente Wi-Fi não consegue transportar o encaminhamento de portas), Modo de Desenvolvedor ativado no dispositivo, um checkout do WebDriverAgent em `~/Library/Caches/dsh-ios/wda/src` (o plugin compila o scheme `WebDriverAgentRunner` a partir daí — ele nunca baixa nem clona nada) e o `iproxy` do libimobiledevice (`brew install libimobiledevice`) para os túneis USB. A primeira compilação do WDA instala um WebDriverAgentRunner assinado: confie no certificado dele no dispositivo quando solicitado e execute `ios_real_start_wda` novamente quando o perfil de assinatura de equipe gratuita expirar (validade de 7 dias).
...
- **Chamadas de dispositivo real falham com um status codificado** — o status do painel nomeia a causa em vez de adivinhar: `device-locked` (desbloqueie o telefone; ele se recupera sozinho), `cert-untrusted` (confie no certificado do WebDriverAgent no dispositivo), `profile-expired` (a assinatura de equipe gratuita dura 7 dias — execute `ios_real_start_wda` novamente para recompilar), `tunnel-failed` (verifique o link USB/iproxyd), `device-unplugged` (use um cabo USB com capacidade de dados — um pareamento somente Wi-Fi não consegue transportar o encaminhamento de portas).
