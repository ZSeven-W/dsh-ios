...
<p align="center">
  ...
  <sub>21 herramientas del agente &bull; panel lateral MJPEG en vivo &bull; simulador &amp; iPhone real por USB &bull; acciones sobre filas de listas/feeds &bull; recarga en caliente de vistas previas SwiftUI</sub>
</p>

<p align="center">
  <sub>npm: <code>@zseven-w/dsh-ios</code> &middot; Versión actual del plugin: <code>0.1.0-rc.1</code> &middot; Probado con DSH <code>0.1.0-rc.6</code></sub>
</p>
...
<p align="center">
  <sub>rc: <code>0.1.0-rc.1</code> aún no está publicado en npm — consulta <a href="#instalar-en-dsh">Instalar</a></sub>
</p>
...
| 🔐 **Transporte solo por loopback** | serve-sim se enlaza a 127.0.0.1 en un rango de puertos dedicado; cada ruta exige un par loopback, un `Host` loopback y comprobaciones Fetch-Metadata/Origin; las capacidades HMAC caducan en 10 minutos. Los túneles de control/MJPEG de WebDriverAgent en un dispositivo real son reenvíos loopback de `iproxy` tras la misma barrera. |
...
- **Transporte de dispositivo real**: los túneles de control de WebDriverAgent (REST, puerto de dispositivo 8100) y pantalla (MJPEG, puerto 9100) son reenvíos loopback de `iproxy` sobre el enlace USB; quedan tras la misma barrera de rutas firmadas, y el navegador sigue hablando solo con el origen del servidor web de DSH.
...
- **DSH ≥ 0.1.0-rc.6 con el bundle web** para el panel. Los perfiles headless también funcionan: las 21 herramientas operan con normalidad, solo sin la vista en vivo.
- **Hosts que no son macOS**: el plugin carga y las 21 herramientas se registran, pero cada llamada devuelve un error explicativo (`iOS Simulator requires macOS with Xcode …`).
...
- **El attach de lldb necesita el modo desarrollador de macOS**: ejecuta `sudo DevToolsSecurity -enable` una vez. Hasta entonces `ios_sim_backtrace` usa el motor `sample` de Xcode (sin suspender) y `ios_sim_leaks` se degrada con la pista de activación.
- **iPhone real**: un iPhone conectado por USB con la pantalla desbloqueada (WebDriverAgent no puede iniciarse con la pantalla bloqueada; considera Bloqueo automático: Nunca), un cable USB con datos (un emparejamiento solo por Wi-Fi no puede transportar el reenvío de puertos), modo desarrollador activado en el dispositivo, una copia de WebDriverAgent en `~/Library/Caches/dsh-ios/wda/src` (el plugin compila su esquema `WebDriverAgentRunner` desde ahí: nunca descarga ni clona nada) e `iproxy` de libimobiledevice (`brew install libimobiledevice`) para los túneles USB. La primera compilación de WDA instala un WebDriverAgentRunner firmado: confía en su certificado en el dispositivo cuando se te pida y vuelve a ejecutar `ios_real_start_wda` cuando caduque el perfil de firma de equipo gratuito (validez de 7 días).
...
> **Nota rc**: `0.1.0-rc.1` aún no está publicado en npm. Hasta entonces, instala el tarball empaquetado:
>
> ```sh
> npm pack                                   # en este repositorio → dsh-ios-0.1.0-rc.1.tgz
...
- **Las llamadas a dispositivo real fallan con un estado codificado**: el estado del panel nombra la causa en lugar de adivinarla: `device-locked` (desbloquea el teléfono; se recupera solo), `cert-untrusted` (confía en el certificado de WebDriverAgent en el dispositivo), `profile-expired` (la firma de equipo gratuito dura 7 días: vuelve a ejecutar `ios_real_start_wda` para recompilar), `tunnel-failed` (revisa el enlace USB/iproxyd), `device-unplugged` (usa un cable USB con datos: un emparejamiento solo por Wi-Fi no puede transportar el reenvío de puertos).
